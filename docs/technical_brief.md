...
Exhibits 1, 4, and 5 are pipeline outputs (`output/`, `logs/`) rather than files shipped in the repository — both directories are gitignored by design (see [README.md](../README.md)). They are regenerated by running `./scripts/run_all.sh` followed by `do do/age_earnings_CPS2015.do`, per the reproduction steps in the README. Exhibits 2 and 3 are committed to the repository as Python reproductions (`scripts/export_figures.py`, since Stata's `.gph` format can't be rendered without a Stata license — see README, "Figures").

- Exhibit 1 — Model comparison table (all four specifications): `output/tables/model_comparison.txt`
- Exhibit 2 — Marginal effect of age on predicted wage: [`docs/figures/figure1_margins_age_profile.png`](figures/figure1_margins_age_profile.png)
- Exhibit 3 — AHE distribution by education and gender: [`docs/figures/figure2_box_ahe_edu_gender.png`](figures/figure2_box_ahe_edu_gender.png)
