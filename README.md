...
`do/age_earnings_CPS2015.do` writes `.gph` graphs (Stata's binary
format) to `output/graphs/`, which is gitignored and can't be rendered
without Stata. The two figures below are Python reproductions from the
same `data/analysis/cps_2015_analysis.dta` file, generated by
`scripts/export_figures.py` — same rationale as
`scripts/regress_baseline.py` above.

**Figure 1** — marginal effect of age on predicted ln(AHE), from the
quadratic specification (`age + age² + female + bachelor +
female×bachelor`, robust SE). Reproduces `margins ... dydx(age)` +
`marginsplot` (do-file spec 10).

![Marginal effect of age on predicted wage](docs/figures/figure1_margins_age_profile.png)

**Figure 2** — distribution of AHE by education and gender. Reproduces
`graph box ahe, over(bachelor) over(female) asyvars` (do-file spec 11).

![AHE by education and gender](docs/figures/figure2_box_ahe_edu_gender.png)
...
└── docs/
    ...
    └── figures/                                # Python reproductions of the do-file's .gph graphs
        ├── figure1_margins_age_profile.png
        └── figure2_box_ahe_edu_gender.png
```
